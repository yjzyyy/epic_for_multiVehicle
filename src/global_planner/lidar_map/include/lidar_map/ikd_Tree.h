...
// #include "lidar_map/sdf_map.h"
// #include <lidar_map/sdf_map.h>

#define EPSS 1e-6
// #define Minimal_Unbalanced_Tree_Size INFINITY
...
struct PointHash {
  std::size_t operator()(const Eigen::Vector3f p) const {
    std::size_t seed = 0;
...
private:
  ...
  // void Search_by_radius_and_delete(KD_TREE_NODE *root, PointType point, float radius, PointVector &Storage);
  // void Search_by_radius_and_delete_frt(KD_TREE_NODE *root, PointType point, float radius, PointVector &Storage, unordered_map<int, Eigen::Vector3f> &frt_norm, Eigen::Vector3f &map_min_bd_, double &inv_cell_size_);
...
public:
  KD_TREE(float delete_param = 0.5, float balance_param = 0.6, float box_length = 0.05);
  ~KD_TREE();

  void setMap_ikdtree(fast_planner::SDFMap *sdf);
  void setMap_ikdtree(fast_planner::LIOInterface *lidar_map);

  bool del_point(PointType pt);
  bool del_point_frt(PointType pt, PointType pt_center, unordered_map<int, Eigen::Vector3f> &frt_norm, Eigen::Vector3f &map_min_bd_, double &inv_cell_size_);
  fast_planner::SDFMap *sdf_;
  fast_planner::LIOInterface *lidar_map_interface_;
  Eigen::Vector3f l_cencen;
  ...
  void InitializeKDTree(float delete_param = 0.5, float balance_param = 0.6, float box_length = 0.05);
  int size();
  int validnum();
  ...
  void Radius_Delete(PointType point, const float radius, PointVector &Storage);
  void Radius_Delete_Frt(PointType point, const float radius, PointVector &Storage, unordered_map<int, Eigen::Vector3f> &frt_norm, Eigen::Vector3f &map_min_bd_, double &inv_cell_size_);
  // void Radius_Delete(PointType point, const float radius, PointVector &Storage, shared_ptr<fast_planner::SDFMap> sdfptr_);
  // bool delete_point(PointType point,  std::shared_ptr<fast_planner::SDFMap> sdfptr_);
  // bool (SDFMap::*ptr_delete_point)(PointType point);
  // ptr_delete_point = &SDFMap::delete_point;
  ...
  friend fast_planner::LIOInterface;
};
