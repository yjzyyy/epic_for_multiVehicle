...
string trajFile;
string mapFile;
double overallMapVoxelSize = 0.2;
...
float vehicleYaw = 0;
float vehicleX = 0, vehicleY = 0, vehicleZ = 0.1;
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn)
{
  if (!systemDelayInited) {
    ...
    if (systemDelayCount > systemDelay) {
      ...
      std::cout << "systemDelayInited set to true!" << std::endl;
    }
  }
  ...
  laserCloud->clear();
  // std::cout << "before pcl::fromROSMsg" << std::endl; // 新增
  pcl::fromROSMsg(*laserCloudIn, *laserCloud);
  // std::cout << "after pcl::fromROSMsg, cloud size: " << laserCloud->size() << std::endl; // 新增
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2> ("/drone_0_ego_planner_node/grid_map/occupancy_inflate", 5, laserCloudHandler);
  ros::Subscriber subLaserCloud_ground = nh.subscribe<sensor_msgs::PointCloud2> ("/vehicle_0_pcl_render_node/cloud", 5, laserCloudHandler);

  ros::Subscriber subRuntime = nh.subscribe<std_msgs::Float32> ("/runtime", 5, runtimeHandler);

  ros::Publisher pubOverallMap = nh.advertise<sensor_msgs::PointCloud2> ("/map_generator/global_cloud", 5);
  ...
  std::cout<<"size of voxle:"<<overallMapVoxelSize<<std::endl;
