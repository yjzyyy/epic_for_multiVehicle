...
void odometryHandler(const nav_msgs::Odometry::ConstPtr& odom)
{
  ...
  if (sendTF) {
    if (!reverseTF) {
      ...
      tfBroadcasterPointer->sendTransform(tf::StampedTransform(odomTrans.inverse(), odom->header.stamp, "sensor", "world"));
    }
  }
}
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn)
{
  ...
  laserCloud2.header.frame_id = "world";
...
int main(int argc, char** argv)
{
  ...
  ros::Publisher pubLaserCloud = nh.advertise<sensor_msgs::PointCloud2> ("/vehicle_0_pcl_render_node/cloud", 5);
